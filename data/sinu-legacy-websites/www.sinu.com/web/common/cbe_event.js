...
/*    || (eventType.indexOf('resize') != -1 && !is.nav4 && !is.opera5or6)
    || (eventType.indexOf('scroll') != -1 && !is.nav && !is.opera5or6) */
  ) {
    var add=true;
...
function cbeNativeAddEventListener(ele, eventType, eventListener, useCapture) {
  ...
  if (ele.addEventListener) {
    ...
    ele.attachEvent("on"+eventType, eventListener);
  }  
...
function cbeNativeRemoveEventListener(ele, eventType, eventListener, useCapture) {
  ...
  if (ele.removeEventListener) {
    ...
    ele.detachEvent("on"+eventType, eventListener);
  }  
...
function cbePropagateEvent(evt) {
  ...
  if (window.event && _def(window.event.cancelBubble)) window.event.cancelBubble = true;
...
function cbeDefaultResizeListener() {
  if (is.opera5or6) location.replace(location.href);
  else history.go(0);
}
var cbeDragObj, cbeDragTarget, cbeDragPhase;
function cbeDragStartEvent(e) {
  if (is.opera5or6) { var tn = e.target.tagName.toLowerCase(); if (tn == 'a') return; }
